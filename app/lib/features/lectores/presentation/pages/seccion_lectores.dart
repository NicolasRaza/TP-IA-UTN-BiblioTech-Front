...
class _SeccionLectoresState extends State<SeccionLectores> {
  ...
  Widget build(BuildContext context) {
    final estado = context.watch<LectoresBloc>().state;
    final q = _busqueda.text.trim().toLowerCase();
    final lectores = estado.lectores
        .where((l) =>
            q.isEmpty ||
...
class _FilaLector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final activos = context
        ...
        .state
        .todos
        .where((p) => p.lectorId == lector.id && p.estaAbierto)
        .length;
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    if (lector.multasPendientes > 0)
                      FilledButton.icon(
                        onPressed: () => context.read<LectoresBloc>().add(
                              LectorActualizado(
                                lector.copyWith(multasPendientes: 0),
                              ),
                            ),
  ...
  static Future<void> _cambiarCategoria(
    ...
    context
        .read<LectoresBloc>()
        .add(CategoriaCambiada(lectorId: lector.id, categoria: nueva));
  }
}
