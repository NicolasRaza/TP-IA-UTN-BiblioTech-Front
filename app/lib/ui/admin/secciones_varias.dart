...
class _SeccionAuditoriaState extends State<SeccionAuditoria> {
  ...
  Widget build(BuildContext context) {
    ...
    final eventos = estado.repo.auditoria
                ...
                children: [
                  for (final e in eventos.take(100))
                    ListTile(
                      ...
                      subtitle: Text(
                        '${Formato.fechaHora(e.fecha)} · ${estado.repo.lector(e.usuarioId)?.nombreCompleto ?? e.usuarioId}',
...
class SeccionAprendizaje extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final estado = context.watch<AppState>();
    final reporte = estado.aprendizaje.generarReporte();
    final reco = estado.aprendizaje.analizarRecomendaciones();
...
class SeccionSugerencias extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final estado = context.watch<AppState>();
    final sugerencias = estado.evaluador.generarSugerencias();
  ...
  static String _titulo(String tipo) => switch (tipo) {
        'compra' => 'Reforzar stock',
        'promocion' => 'Promoción',
        _ => 'Política de préstamos',
      };

  static IconData _icono(String tipo) => switch (tipo) {
        'compra' => Icons.add_shopping_cart,
        'promocion' => Icons.campaign_outlined,
        _ => Icons.policy_outlined,
      };

  static Color _color(String tipo) => switch (tipo) {
        'compra' => Paleta.teal,
        'promocion' => Paleta.gold,
        _ => Paleta.pink,
      };
}
...
class SeccionAcerca extends StatelessWidget {
                ...
                children: [
                  const Text('Bitácora de esta sesión',
                      style: TextStyle(
                          ...
                          color: Paleta.textPrimary)),
                  const SizedBox(height: 10),
                  for (final linea in estado.bitacora.reversed.take(15))
                    Padding(
  ...
  static Future<void> _confirmarReinicio(BuildContext context) async {
    ...
    context.read<AppState>().reiniciarDatos();
  }
}
